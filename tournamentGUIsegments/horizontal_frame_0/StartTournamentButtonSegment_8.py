...
class StartTournamentButtonSegment_8(AbstractGUISegment):
    ...
    def start_tournament(self):
        ...
        row_widgets = self.get_all_gui_widgets()[0]

        optionMenu_protocol_var = self.get_special_StringVar(0, 0)
        selected_protocol = optionMenu_protocol_var.get()
        if selected_protocol == 'Select a protocol':
            message += 'Protocol, '

        selected_analysis_string_var = self.get_special_StringVar(1, 0)
        selected_analysis = selected_analysis_string_var.get()
        if selected_analysis == 'Select an analysis':
            message += 'Analysis, '

        selected_tournament_analysis_string_var = self.get_special_StringVar(2, 0)
        ...
        deadline_var_tuple = self.get_current_horizontal_frame_var_dict()[5]
        deadline_var = deadline_var_tuple[0]
        deadline = deadline_var.get()

        deadline_type_var = self.get_special_StringVar(5, 1)
        ...
        if not self.__first_clicked:
            ...
            self.add_horizontal_frame(horizontal_index=index, frame=new_horizontal_frame)
            self.create_tournament_visualization_window3(new_horizontal_frame)
        else:
            h_frame_alternative1 = tk.Frame(master=self.get_root())
            self.replace_special_horizontal_frame(horizontal_index=index - 1, frame=h_frame_alternative1)
